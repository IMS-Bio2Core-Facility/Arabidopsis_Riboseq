...
WORKDIR="/usr/data/bgfs1/maloku/RNASeq/bam_file/";
#dir.create(WORKDIR)
setwd(WORKDIR)
Results="/usr/data/bgfs1/maloku/RNASeq/Results/"; 
#dir.create(Results)
...
fc <-featureCounts(files=BAMS, annot.ext="/usr/data/bgfs1/maloku/RNASeq/Arabidopsis_thaliana.TAIR10.45.gtf",
...
#plotMD(d, column=11)
#abline(h=0, col="red", lty=2, lwd=2)
#write.csv(d$counts, file="/usr/home/maloku/RiboProfiling_test/filtering_of_low_expr_genes_counts.csv")
#write.csv(d$samples, file="/usr/home/maloku/RiboProfiling_test/filtering_of_low_expr_genes_samples.csv")

# Setting up the  model

targets <-readTargets("/usr/data/bgfs1/maloku/RNASeq/sample_rnaseq.csv",sep=",")
time <-targets$Time
...
#fit <- glmFit(dgList, mm)
#lrt <- glmLRT(fit, contrast = T1vsT9 )
#topTags(lrt)
...
#is <- decideTestsDGE(lrt)
#summary(is)

fit1 <- glmQLFit(dgList, mm,robust=TRUE)
...
plotMD(qlf.T1vsT21, status=is.de_T1vsT21, values=c(1,-1), col=c("red","blue"), legend="topright")
...
o <- order(qlf.T1vsT23$table$PValue)
logCPM <- logCPM[o[1:100],]
logCPM <- t(scale(t(logCPM)))
my_palette <- colorRampPalette(c("blue", "white", "red")) 
heatmap.2(logCPM,col=rev(my_palette(70)),trace="none", main="Top 30 most variable genes across samples for T1vsT3",scale="row")
